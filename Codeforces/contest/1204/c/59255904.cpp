...
int main() {
    ...
    int M=m;
    while(middle<M-1){
        cerr<<front<<" "<<middle<<" "<<rear<<endl;
        cerr<<"Data "<<vec[front]<<" "<<vec[middle]<<" "<<vec[rear]<<endl;
        cerr<<"Dist "<<d[vec[front]][vec[middle]]<<" "<<d[vec[middle]][vec[rear]]<<" "<<d[vec[front]][vec[rear]]<<endl;
        if(vec[front]!=vec[rear]&&d[vec[front]][vec[middle]]+d[vec[middle]][vec[rear]]==d[vec[front]][vec[rear]]){
            vec[middle]=-1;
            m--;
            cerr<<"Delete Middle"<<endl;
        }else{
            front=middle;
            cerr<<"Keep Middle"<<endl;
        }
        middle++;
        rear++;
    }
